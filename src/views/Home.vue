...
export default {
    ...
    methods: {
        ...
        go() {
            if (this.id == null) {
                window.alert("請選擇要更新的新聞")
            } else {
                this.$router.push(`/updata/${this.id}`);
            }
        },
        changeOpen() {
            let body = {
            ...
            fetch("http://localhost:8080/change_open", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
...
<template>
    <div id="wrap" class="d-flex flex-column mb-4 mt-2">
        ...
        <div class="d-flex justify-content-between">
            <div class="mt-2">
                <button @click="changeOpen" class="ms-5" type="button">開放/隱藏新聞</button>
                <button @click="add" class="ms-5" type="button">新增</button>
            </div>
            <div class="mt-2 me-5">
                <button @click="go" class="ms-5" type="button">更新新聞</button>
            </div>
        </div>
        <div class="Result">
            <table class="table table-danger table-striped table-bordered border border-danger">
                ...
                <tbody>
                    <!-- 使用子元件並傳遞相關資料 -->
                    <Result v-for="(property, index) in items" @getTarget="enter" v-bind:key="property"
